...
def kfold(D, L, fold, params, app):
    ...
    for j in range(fold):
        ...
        llr_final.extend(tied_GMM(DTR, LTR, DTE, params))
    ...
    print("\-/ \-/ \-/ \-/ \-/ ")
    print(f"Tied GMM error: {round(error, 3)}")
...
def tied_GMM(DTR, LTR, DTE, n_repetions):
    N = DTR.shape[1]
    t = 1e-6
    w = 1.0
    alpha = 0.1
    ...
    for i in numpy.unique(LTR):
        ...
        for j in range (n_repetions):
            ...
            for k, g in enumerate(GMM_init.values()):
                w, mu, Cov = g
                newW = w/2
                if(len(GMM_init.values()) > 2):
                    Z = 0
                    newC = 0
                    for l in range(resps.shape[0]):
                        Z = numpy.sum(ml.mrow(resps[l, :]), axis=1)
                        newC += Z*Cov
                    Cov = 1/N*newC
                U, s, Vh = numpy.linalg.svd(Cov)
                s[s<psi] = psi
                Cov = numpy.dot(U, ml.mcol(s)*U.T)
            ...
            GMM_final, opt_ll, resps = ml.optimize_GMM_Tied(DTE, GMM_final)
...
kfold(data_g, labels, 5, 5, [0.5, 1, 1])
