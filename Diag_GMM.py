...
def Diag_GMM(DTR, LTR, DTE, n_repetions):
    N = DTR.shape[1]
    t = 1e-6
    w = 1.0
    alpha = 0.1
    ...
    for i in numpy.unique(LTR):
        C[i] = C[i]*numpy.eye(C[i].shape[0])
        U, s, _ = numpy.linalg.svd(C[i])
        s[s<psi] = psi
        C[i] = numpy.dot(U, ml.mcol(s)*U.T)
        gmm[i] = [w, mu[i], C[i]]
        GMM_final = {0 : gmm[i]}
...
def kfold(D, L, fold, params, app):
    ...
    for j in range(fold):
        ...
        llr_final.extend(Diag_GMM(DTR, LTR, DTE, params))
    ...
    print("\-/ \-/ \-/ \-/ \-/ ")
    print(f" Diag GMM error: {round(error, 3)}")
...
kfold(data_z, labels, 5, 5, [0.5, 1, 1])
