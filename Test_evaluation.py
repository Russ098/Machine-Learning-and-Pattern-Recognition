...
DTE, LTE = ml.loadFile('./Test.txt')


# In[3]:


DTR_g = ml.gaussianize(Data)
DTE_g = ml.gaussianize(DTE)
...
def kfold1(D, L, fold, params, app):
    error = 0
    ...
    minDCF, optThreshold= ml.compute_min_DCF(llr_final, app, LTE_final)
...
def final_elab(DTR, LTR, DTE, LTE, params, app, optThreshold):
    
    llr_final= radialSVM(DTR, LTR, DTE, params)
        
    CM = ml.compute_optimal_B_decision(app, llr_final, LTE, optThreshold)

    app_bayes_risk=ml.compute_Bayes_risk(CM, app)
    DCF = ml.compute_norm_Bayes(app_bayes_risk, app)
    
    error = 1-(CM[0, 0]+CM[1,1])/(len(LTE))
    sensitivity = 1-ml.compute_FNR(CM)
    specificity = 1-ml.compute_FPR(CM)

    print("\-/ \-/ \-/ \-/ \-/ ")
    print(f"Radial Basis SVM error: {round(error, 3)}")
    print(f'{app} DCF:{round(DCF, 3)}')
...
t = kfold1(DTR_g, labels, 5, [1, 1, 10], [0.5, 1, 1])


# In[9]:


final_elab(DTR_g, labels, DTE_g, LTE, [1, 1, 10], [0.5, 1,1], t)
