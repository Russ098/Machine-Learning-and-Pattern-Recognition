...
def EM(X, ref_GMM, resps):
    ...
    logJoint = logpdf_GMM(X, ref_GMM)
    logMargDen = comp_log_marg(logJoint)
    succLL = numpy.sum(logMargDen) / N
    resps = comp_posteriors(logJoint, logMargDen)

    return ref_GMM, logJoint, succLL, resps
...
def logpdf_GAU_ND(X, mu, C):
   ...
   sig, detSigm = numpy.linalg.slogdet(C)
   second = -0.5*detSigm
   third = -0.5*numpy.dot(numpy.dot(((X-mu).T), numpy.linalg.inv(C)), (X-mu))
...
def comp_log_marg(J):
    ...
    log_dens = scipy.special.logsumexp(J, axis=0)

    return log_dens

def comp_posteriors(logJoint, logMargDen):

    posteriors = logJoint-logMargDen

    return numpy.exp(posteriors)


def optimize_GMM(X, GMM, t=1e-6):
    N = X.shape[1]
    
    logJoint = logpdf_GMM(X, GMM)
    logMargDen = comp_log_marg(logJoint)
    prevLL = numpy.sum(logMargDen)/N
    resps = comp_posteriors(logJoint, logMargDen)

    GMM, logJoint, succLL, resps = EM(X, GMM, resps)
...
def optimize_GMM_Tied(X, GMM, t=1e-6):
    N = X.shape[1]
